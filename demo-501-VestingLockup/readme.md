...
锁仓释放，英文名称为 “Vesting with Lock-up”，是指代币在一段时间内无法被转移或销售，而只能在指定的时间点或时间段后才能被释放或转移。

这种机制常常用于 **`ICO`**（首次代币发售）或团队成员的代币，以确保他们在一定时间内不会抛售，从而保持代币价格的稳定性。

锁仓释放机制，有时候会直接写在 **`ERC20`** 代币合约中，有时候也会单独编写为一个合约。

在 **`OpenZeppelin`** 合约库中有一个 **`VestingWallet`** 合约，就是一个通用的锁仓释放合约。

我们将其简化，单独编写一个简单的锁仓释放合约。
